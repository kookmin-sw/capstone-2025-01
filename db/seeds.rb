...
if Rails.env.development?
  ...
  # 추가 테스트 법안 생성 (기존 seed.rb 파일 끝에 추가)
  # 추가 법안 제목 목록
  additional_bill_titles = [
    "청소년 보호법 일부개정법률안",
    "국가균형발전 특별법 일부개정법률안",
    "여성농어업인 육성법 일부개정법률안",
    "식품안전기본법 일부개정법률안",
  ...
  # 기존 의원들을 활용
  assembly_proposers = NationalAssemblyPerson.all.to_a
  ...
  # 법안 상태 목록
  statuses = ["소관위접수", "본회의 심의 중", "소관위 심사", "정부이송", "공포"]
  
  # 카테고리 목록
  categories = ["경제", "환경", "사회", "과학기술", "복지", "교육", "문화", "외교", "안보", "행정"]
  
  # 추가 법안 생성
  additional_bill_titles.each_with_index do |title, index|
    # 이미 존재하는 법안인지 확인
    next if Bill.exists?(title: title)
    
    # 대표 발의자와 공동 발의자 선택
    rep_proposer = assembly_proposers[index % assembly_proposers.size]
    co_proposers = assembly_proposers.reject { |p| p.id == rep_proposer.id }.sample(rand(3..6))
    
    # 테스트 법안 생성
    bill = Bill.create!(
      title: title,
      bill_number: "2240#{index + 100}",
      bill_type: "법률안",
      assembly_bill_id: "TEST_#{rand(10000..99999)}",
      summary: "제안이유 및 주요내용: 본 법안은 #{title.gsub('법률안', '').gsub('개정안', '')}에 관한 사항을 규정하여 국민의 삶의 질 향상과 국가 경쟁력 강화에 기여하고자 함. 구체적인 내용은 다음과 같다. 1. #{title.gsub('법률안', '').gsub('개정안', '')} 관련 제도 개선, 2. 관련 인프라 확충 지원, 3. 국민 편의 증진을 위한 서비스 확대 등",
      proposed_at: Date.today - rand(10..300).days,
      committee_name: committees.sample,
      bill_stage: statuses.sample,
    ...
    Proposal.create!(
      bill: bill,
      specific_proposer: rep_proposer,
      representative_proposal: true
    )
    
    # 공동 발의자 생성
    co_proposers.each do |proposer|
      Proposal.create!(
        bill: bill,
        specific_proposer: proposer,
        representative_proposal: false
      )
    end
    ...
    BillDetail.create!(
      bill_id: bill.id,
      jurisdiction_examination_xml: "<committeeName>#{bill.committee_name}</committeeName><presentDt/><procDt/><procResultCd/><submitDt>#{bill.proposed_at + 1.day}</submitDt>",
      law_title: title.split(" ").first(3).join(" ")
    )
    
    puts "Created test bill: #{title}"
  end
  
  puts "Total bills: #{Bill.count}"
